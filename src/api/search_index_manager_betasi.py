...
class SearchIndexManager:
    ...
    MIN_LINE_LENGTH = 5
    DEFAULT_OUTPUT_FILE_NAME = "/content.csv.data"
    ...
    async def search(self, message: ChatRequest) -> str:
        ...
        embed_response = (await self._embeddings_client.embed(
            input=message.messages[-1].content,
            dimensions=self._dimensions,
            model=self._model
        ))
        embedded_question = embed_response['data'][0]['content_embedding']
        embedded_source = embed_response['data'][0]['content_source']
        
        vector_query = VectorizedQuery(vector=embedded_question, k_nearest_neighbors=5, fields="content_embedding")
        response = await self._get_client().search(
            vector_queries=[vector_query],
            select=['content_data', 'content_source'],
        )
        results = [result['content_data'] async for result in response if result['content_source'].upper() == embedded_source.upper()]
        return "\n------\n".join(results)
    ...
    async def upload_documents(self, embeddings_file: str) -> None:
        ...
        documents = []
        index = 0
        logger.info(f"Uploading documents from {embeddings_file} to index {self._index_name}.")
        with open(embeddings_file, encoding='utf-8', newline='') as fp:
            reader = csv.DictReader(fp)
            for row in reader:
                documents.append(
                    {
                        'content_id': hashlib.md5((row['content_data'] + row['content_source'] + str(index)).encode()).hexdigest(),
    ...
    async def _index_create(
        ...
        async with SearchIndexClient(endpoint=endpoint, credential=credential) as ix_client:
            fields = [
                ...
                SimpleField(name="content_source", type=SearchFieldDataType.String, hidden=False),
                SimpleField(name="content_data", type=SearchFieldDataType.String, hidden=False),
            ]
            ...
            new_index = await ix_client.create_or_update_index(search_index)
        return new_index
    ...
    async def build_embeddings_file(
        ...
        index = 0
        for fle in globs:
            relative_path = os.path.relpath(os.path.dirname(fle), input_directory)
            with open(fle, encoding='utf-8') as f:
                for line in f:
                    ...
                    if len(line) < SearchIndexManager.MIN_LINE_LENGTH or len(set(line)) < SearchIndexManager.MIN_DIFF_CHARACTERS_IN_LINE:
                        index = 0
                        continue
                    for sentence in sent_tokenize(line, language = "polish"):
                        if index % sentences_per_embedding == 0:
                            sentence_tokens.append([sentence, relative_path])
                        else:
                            sentence_tokens[-1][0] += ' '
                            sentence_tokens[-1][0] += sentence
                        index += 1
        ...
        if output_file is None:
            output_file = input_directory + SearchIndexManager.DEFAULT_OUTPUT_FILE_NAME

        with open(output_file, mode='w', encoding='utf-8') as fp:
            writer = csv.DictWriter(fp, fieldnames=['content_source', 'content_data', 'content_embedding'])
            writer.writeheader()
            for i in range(0, len(sentence_tokens), batch_size):
                emedding = (await self._embeddings_client.embed(
                    input=[s[0] for s in sentence_tokens[i:i+min(batch_size, len(sentence_tokens))]],
                    dimensions=self._dimensions,
                    model=self._model
                ))["data"]
                for token, float_data in zip(sentence_tokens, emedding):
                    writer.writerow({'content_source': token[1], 'content_data': token[0], 'content_embedding': json.dumps(float_data['embedding'])})
